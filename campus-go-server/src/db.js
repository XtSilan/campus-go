...
function hasAnyFollows() {
  const row = db.prepare(`SELECT COUNT(*) AS count FROM follows`).get()
  return row.count > 0
}
...
function ensureNotificationSeeds() {
  ...
  const seeds = [
    {
      title: '平台提醒',
      content: '校园购仅提供信息展示与沟通渠道，不包含支付与担保服务，请务必线下核验物品。',
      type: 'warning',
      audience: 'all',
      status: 'active',
      pinned: 1,
    ...
    {
      title: '本周推荐',
      content: '临近期末，教材、显示器、宿舍风扇和代拿服务的需求明显增加，发布时建议写清校区和交接时间。',
      type: 'info',
      audience: 'all',
      status: 'active',
      pinned: 0,
    ...
    {
      title: '社区规范',
      content: '禁止发布违法违规、校外无关或明显不实内容，管理员会定期巡检并处理异常信息。',
      type: 'success',
      audience: 'all',
      status: 'active',
      pinned: 0,
  ...
  for (const item of seeds) {
    createNotificationRecord(item, adminId)
  }
}

function ensureFollowSeeds() {
  if (hasAnyFollows()) {
    return
  }
  ...
  db.prepare(`INSERT OR IGNORE INTO follows (user_id, followed_user_id, created_at) VALUES (?, ?, ?)`).run(users[0].id, users[1].id, now())
  db.prepare(`INSERT OR IGNORE INTO follows (user_id, followed_user_id, created_at) VALUES (?, ?, ?)`).run(users[0].id, users[2].id, now())
  db.prepare(`INSERT OR IGNORE INTO follows (user_id, followed_user_id, created_at) VALUES (?, ?, ?)`).run(users[1].id, users[0].id, now())
}
...
export function seedDatabase() {
  if (!hasAnyUsers()) {
    ...
    const userIds = demoUsers.map(item => createUser({ status: 'active', role: item.role || 'user', ...item }).id)
    const listings = [
      {
        ownerId: userIds[0],
        type: 'supply',
        title: '高等数学教材上下册，笔记完整',
        category: '书籍教材',
        condition: '八成新',
        description: '大一下学期使用过，重点和例题都做了标注，适合期末复习直接用。',
        price: 28,
        imageUrl: '',
        location: '图书馆东门',
        contactName: '林知夏',
      ...
      },
      {
        ...
        category: '生活用品',
        condition: '九成新',
        description: '去年夏天买的，风力够用，适合宿舍书桌，插电即用。',
        price: 35,
        imageUrl: '',
        location: '南门快递站',
        contactName: '周一南',
      ...
      },
      {
        ...
        category: '电子产品',
        condition: '有使用痕迹',
        description: '轴体手感正常，键帽有轻微打油，适合宿舍办公和写论文。',
        price: 89,
        imageUrl: '',
        location: '一食堂门口',
        contactName: '周一南',
      ...
      },
      {
        ...
        category: '代拿外卖',
        condition: '可正常使用',
        description: '每天 18:30 到 21:00 可接单，主打女生宿舍区，熟悉各楼栋。',
        price: 3,
        imageUrl: '',
        location: '女生宿舍楼下',
        contactName: '何予安',
        phone: '',
        wechat: 'heyuan_service',
        qq: '',
      },
      {
        ownerId: userIds[0],
        ...
        category: '运动器材',
        condition: '九成新',
        description: '很少用，拍线完好，两个拍套都在，适合情侣或室友一起打。',
        price: 58,
        imageUrl: '',
        location: '体育馆门口',
        contactName: '林知夏',
      ...
      },
      {
        ownerId: userIds[2],
        type: 'demand',
        title: '求购考研英语黄皮书 2026 版',
        category: '书籍教材',
        condition: '可正常使用',
        description: '希望有近两年的真题解析版本，内部标注不要太重。',
        price: 40,
        imageUrl: '',
        location: '东区教学楼',
        contactName: '何予安',
      ...
      },
      {
        ownerId: userIds[0],
        type: 'demand',
        title: '想收一台旧显示器，宿舍写代码用',
        category: '电子产品',
        condition: '可正常使用',
        description: '尺寸 21 到 24 寸都可以，接口正常，价格不要太高。',
        price: 120,
        imageUrl: '',
        location: '本部校区宿舍',
        contactName: '林知夏',
      ...
      },
      {
        ...
        category: '代拿快递',
        condition: '可正常使用',
        description: '晚上 8 点以后都可以送到 5 号宿舍楼，价格好商量。',
        price: 5,
        imageUrl: '',
        location: '南区 5 号楼',
        contactName: '周一南',
    ...
    for (const item of listings) {
      createListingRecord(item.ownerId, item)
    }
  }
